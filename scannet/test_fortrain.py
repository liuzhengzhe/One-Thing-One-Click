...
def test(model, model_fn, data_name, epoch):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            #full_data=torch.load('../data/train_fully/'+test_scene_name+'_vh_clean_2.ply_inst_nostuff.pth')
            ...
            for i in range(cnt):
              ...
              c=semantic_pred[i]
              #print (semantic_pred, i)              
              if conf<conf_thres:
                continue
              cnt_group+=1
              #print (c,i, groups, group_all)              
            ...
            print (len(np.where(sem_labels!=-100)[0]),len(np.where(prev_sem!=-100)[0])) #, len(np.where(full_sem!=-100)[0]))
            #print (cnt_group,prev_group_cnt,full_group_cnt)
            torch.save((coords, colors, sem_labels, groups, prev_seg), 'train_cuda/'+test_scene_name+'_vh_clean_2.ply_inst_nostuff.pth')
