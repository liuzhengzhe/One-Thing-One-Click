...
def test(model, model_fn, data_name, epoch):
    ...
    dataset.testLoader('/media/lzz/faef702d-ca59-4dad-a70a-089cc782183e1/pg/scannetv2/test/') #'../data/val_cuda/')
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            '''if cfg.eval:
                ...
                gt_file = os.path.join('../data/', cfg.split + '_gt', test_scene_name + '.txt')
            ...
            #print ('cluster', clusters.shape,            cluster_scores.shape,             cluster_semantic_id.shape,  N)
            
                                    
            
            
            f=open('instance_result/'+test_scene_name+'.txt', 'w')
         
            for i in range(clusters.shape[0]):

              #print (i)
              #print (cluster_semantic_id[i].item())
              #print (semantic_label_idx[cluster_semantic_id[i].item()])
              #print (cluster_scores[i])
              f.write('predicted_masks/'+test_scene_name+'_'+str(i).zfill(3)+'.txt '+str(int(cluster_semantic_id[i].item()))+' '+str(format(cluster_scores[i].item(),'.4f'))+'\n')
              fm=open('instance_result/predicted_masks/'+test_scene_name+'_'+str(i).zfill(3)+'.txt','w')

              instances=np.zeros((N,1))   
              idxs=np.where(clusters[i,:].cpu().numpy()==1)[0]
              instances[idxs]=1
              for j in range(labels.shape[0]):
                fm.write(str(int(instances[j].item()))+'\n')
              fm.close()
