...
def train_epoch(train_loader, model, model_fn, optimizer, epoch):
    ...
    for i, batch in enumerate(train_loader):
        ...
        loss, semantic_loss_pred, semantic_loss_transformer, semantic_loss_pair, semantic_loss_unary ,offset_norm_loss, offset_dir_loss, visual_dict, meter_dict = model_fn(batch, model, epoch)
        ...
        sys.stdout.write(
            "epoch: {}/{} iter: {}/{} loss:  unary_point:{:.4f},  transformer_sv:{:.4f},   pair_sv:{:.4f},   unary_sv:{:.4f}, offset_norm:{:.4f}, offset_dir:{:.4f},  {:.4f}({:.4f}) data_time: {:.2f}({:.2f}) iter_time: {:.2f}({:.2f}) remain_time: {remain_time}\n".format
            (epoch, cfg.epochs, i + 1, len(train_loader), semantic_loss_pred, semantic_loss_transformer, semantic_loss_pair, semantic_loss_unary ,offset_norm_loss, offset_dir_loss, am_dict['loss'].val, am_dict['loss'].avg,
    ...
    if epoch==1400 or epoch==1600: # or epoch==1024 or epoch==1441 or epoch==1700:
          #os.system('python test.py --config config/pointgroup_run1_scannet.yaml --pretrain exp/scannetv2/pointgroup/pointgroup_run1_scannet/pointgroup_run1_scannet-'+str(epoch).zfill(9)+'_weight.pth')
          #os.system('python evaluate.py --pred_path result/pred1 --gt_path /media/lzz/faef702d-ca59-4dad-a70a-089cc782183e1/pg/gt_val_fully/')



          #os.system('cp -r cnt0.9 cnt0.9_'+str(epoch))      
          os.system('cp -r train_cuda train_cuda_'+str(epoch))    
          
          os.system('CUDA_VISIBLE_DEVICES='+str(cfg.update_gpu)+' python test_fortrain.py --config config/pointgroup_run1_scannet.yaml --pretrain exp/train_cuda/pointgroup/pointgroup_run1_scannet/pointgroup_run1_scannet-'+str(epoch).zfill(9)+'_weight.pth')
...
if __name__ == '__main__':
    ...
    if model_name == 'pointgroup':
      if cfg.first_iter:
        ...
        exit(0)
    print ('cfg.first_iter',cfg.first_iter)
    ...
    #print (start_epoch, 'sssssssssssssssssss')
    #start_epoch=0
