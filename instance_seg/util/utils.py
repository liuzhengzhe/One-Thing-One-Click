...
def checkpoint_restore(model, exp_path, exp_name, use_cuda=True, epoch=0, dist=False, f=''):
    ...
    if not f:
        if epoch > 0:
            ...
            print (os.path.join(exp_path,  '*_weight.pth'))
            f = sorted(glob.glob(os.path.join(exp_path,  '*_weight.pth')))
            if len(f) > 0:
                f = f[-1]
...
def checkpoint_save(model, exp_path, exp_name, epoch, save_freq=16, use_cuda=True):
    ...
    model.cpu()
    torch.save(model.state_dict(), f[:-4]+'_weight.pth')
    torch.save(model.feat, f[:-4]+'_feat.pth')
    if use_cuda:
        model.cuda()

    #remove previous checkpoints unless they are a power of 2 or a multiple of 16 to save disk space
    epoch = epoch - 1
    f = os.path.join(exp_path, exp_name + '-%09d'%epoch + '_feat.pth')
    if os.path.isfile(f):
        if not is_multiple(epoch, save_freq) and not is_power2(epoch):
            os.remove(f)
    f = os.path.join(exp_path, exp_name + '-%09d'%epoch + '_weight.pth')
    if os.path.isfile(f):
        if not is_multiple(epoch, save_freq) and not is_power2(epoch):
            os.remove(f)
