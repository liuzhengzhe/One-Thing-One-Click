...
def read_instance_prediction_file(filename, pred_path):
    ...
    for line in lines:
        parts = line.split(' ')
        if len(parts) != 3:
            util_eval.print_error('invalid instance prediction file. Expected (per line): [rel path prediction] [label id prediction] [confidence prediction]')
        if os.path.isabs(parts[0]):
            util_eval.print_error('invalid instance prediction file. First entry in line must be a relative path')
        ...
        if os.path.commonprefix([mask_file, abs_pred_path]) != abs_pred_path:
            util_eval.print_error('predicted mask {} in prediction text file {} points outside of prediction path.'.format(mask_file,filename))
