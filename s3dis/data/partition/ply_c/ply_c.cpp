...
typedef boost::tuple< std::vector< std::vector<float> >, std::vector< std::vector<uint8_t> >, std::vector< std::vector<uint32_t> >, std::vector<std::vector<uint32_t> >, std::vector<uint32_t> > Custom_tuple;
...
{//prune the point cloud xyz with a regular voxel grid
    ...
    std::vector<uint32_t> point2voxel(n_ver, 0);
    ...
    for (std::size_t i_ver = 0; i_ver < n_ver; i_ver ++)
    {
        ...
        point2voxel[i_ver]=vox_grid.get_index(bin_x, bin_y, bin_z); //vox_grid.index;        
    }
    ...
    return to_py_tuple::convert(Custom_tuple(pruned_xyz,pruned_rgb, pruned_labels,pruned_objects, point2voxel));
}
