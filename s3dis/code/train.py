...
def train_epoch(train_loader, model, model_fn, optimizer, epoch):
    ...
    for i, batch in enumerate(train_loader):
        ...
        loss, semantic_loss_pred, semantic_loss_transformer, semantic_loss_pair, semantic_loss_unary , visual_dict, meter_dict = model_fn(batch, model, epoch)
        ...
        sys.stdout.write(
            "epoch: {}/{} iter: {}/{} loss:  unary_point:{:.4f},  transformer_sv:{:.4f},   pair_sv:{:.4f},   unary_sv:{:.4f}, {:.4f}({:.4f}) data_time: {:.2f}({:.2f}) iter_time: {:.2f}({:.2f}) remain_time: {remain_time}\n".format
            (epoch, cfg.epochs, i + 1, len(train_loader), semantic_loss_pred, semantic_loss_transformer, semantic_loss_pair, semantic_loss_unary , am_dict['loss'].val, am_dict['loss'].avg,
    ...
    if epoch==2500  or epoch==3500 or epoch==4500 or epoch==5500: # or epoch==4500:


          os.system('cp -r cnt0.9 cnt0.9_'+str(epoch))      
          os.system('cp -r train_cuda train_cuda_'+str(epoch))
          #with torch.cuda.device(1):
          #test_fortrain.test_fortrain('config/pointgroup_run1_scannet.yaml','exp/train_cuda/pointgroup/pointgroup_run1_scannet/pointgroup_run1_scannet-'+str(epoch).zfill(9)+'_weight.pth')    
          #cmd=['CUDA_VISIBLE_DEVICES=2 python test_fortrain.py --config config/pointgroup_run1_scannet.yaml --pretrain exp/train_cuda/pointgroup/pointgroup_run1_scannet/pointgroup_run1_scannet-'+str(epoch).zfill(9)+'_weight.pth']
